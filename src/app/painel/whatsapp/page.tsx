...
export default function WhatsappPage() {
  ...
  useEffect(() => {
    if (!tenantId) {
      // Aguardando resolveTenantContext; se não houver sessão, para o loading
      void resolveTenantContext().then((ctx) => {
  ...
  async function startPolling(sessionName: string) {
    ...
    const tick = async () => {
      try {
        ...
        // QR pode falhar enquanto status ainda é STARTING — não interrompe o poll
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-3 rounded-2xl border border-border bg-accent p-4 sm:p-5">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          Aparelhos conectados. A conexão fica vinculada à sua loja de forma
          segura.
        </p>
      </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {conexoes.map((c) => {
            ...
            return (
              <div
                key={c.id}
                className="flex flex-col justify-between rounded-2xl border border-border bg-card p-5 shadow-[var(--shadow-soft)]"
              >
                <div>
